...
def import_xls(request, provider_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            errors = read_xls(header, data, xls_file)
            print(data)
            ...
            if errors:
                msg = u"Les lignes suivantes seront ignorées lors de l'import:\n"
                msg += u"\n".join(errors)
    ...
    for row_idx in range(sheet.nrows):
        ...
        if len(row) >= name_idx + 1:
            ...
            try:
                if len(name) >= 500:
                    is_valid = 'false'
                    errors.append(
                        base_error + u"Colonne 'désignation' - valeur trop longue (%s/500 charactères)" % len(name))
            except:
                is_valid = 'false'
                errors.append(
                    base_error + u"Colonne 'désignation' - impossible de lire la désignation (problère d'accent, etc.)")
        ...
        if len(row) >= pack_idx + 1:
            ...
            if len(pack) >= 100:
                is_valid = 'false'
                errors.append(
                    base_error + u"Colonne 'conditionnement' - valeur trop longue (%s/100 charactères)" % len(pack))
        ...
        if len(row) >= expiry_idx + 1:
            ...
            if expiry_str:
                try:
                    ...
                    is_valid = 'false'
                    errors.append(
                        base_error + u"Colonne 'expiration' - date invalide : %s (format accepté: jj/mm/aaaa)" % expiry_str)
            ...
            if not price:
                ...
                try:
                    ...
                    if price <= 0:
                        is_valid = 'false'
                        errors.append(
                            base_error + u"Colonne 'prix' - le prix est négatif ou nul, il doit être strictement positif.")
                except:
                    is_valid = 'false'
                    errors.append(
                        base_error + u"Colonne 'prix' - impossible de lire une valeur décimale. Valeur reçue: '%s'." %
                        row[price_idx].value)
        else:
            is_valid = 'false'
            errors.append(base_error + u"Colonne 'prix' - valeur manquante.")
...
votre navigateur).")
    ...
    json_data = request.session['import_data']
